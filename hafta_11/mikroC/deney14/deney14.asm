...
;deney14.c,65 :: 		c2ON_bit = 0; //bu bitleri disable etmemiz gerekiyormuþ.
	BCF        C2ON_bit+0, BitPos(C2ON_bit+0)
;deney14.c,67 :: 		Glcd_Init();
	CALL       _Glcd_Init+0
;deney14.c,68 :: 		Glcd_Fill(0x00); // ekraný temizledik.
...
;deney14.c,78 :: 		Glcd_Box(62,40,124,54,1); // Sýnýrlarý (62,40) dan baþlayýp
	...
	CALL       _Glcd_Box+0
;deney14.c,81 :: 		Glcd_Rectangle(5,5,84,35,1); // içi boþ dikdörtgen, 5 e 5 ile 84 e 35 arasýna çiz. 1 siyah.
	MOVLW      5
...
;deney14.c,83 :: 		Glcd_Line(0,0,127,63,1); // Çizgi. 0 a 0 dan baþlayýp 127 ye 63 te biten. 1 siyah.
...
L_main6:
	...
	NOP
	NOP
;deney14.c,92 :: 		Glcd_V_Line(2,54,ii,1); // yatay çizgi çiziyoruz. x koordinatýmýz sabit, y koordinatýmýz deðiþken.
	MOVLW      2
	...
	CALL       _Glcd_V_Line+0
;deney14.c,94 :: 		Glcd_H_Line(2,129,ii,1); // dikay çizgi çiziyoruz.
	MOVLW      2
...
;deney14.c,100 :: 		Glcd_Fill(0x00);
	CLRF       FARG_Glcd_Fill_pattern+0
	CALL       _Glcd_Fill+0
;deney14.c,102 :: 		Glcd_Set_Font(Character8x7, 8, 7, 32);  // 8 e 7 lik karakter, son deðeri offset deðeri.
...
;deney14.c,104 :: 		Glcd_Write_Text("mikroE", 1, 7, 2); // mikroe yazýsý, x koordinatý, y koordinatý, zeminin tersi renginde.
...
;deney14.c,106 :: 		for(ii=1; ii<=10; ii++) Glcd_Circle(63,32,3*ii,1); // koordinatý 63 e 32 olan çapý 3*ii olan ve siyah daire.
	MOVLW      1
	MOVWF      _ii+0
L_main7:
	MOVF       _ii+0, 0
...
;deney14.c,108 :: 		delay2S();
	CALL       _delay2S+0
;deney14.c,110 :: 		Glcd_Box(12,20,70,57,2);  // içi dolu kutu, rengi ters olacak.
	MOVLW      12
	MOVWF      FARG_Glcd_Box_x_upper_left+0
	MOVLW      20
	MOVWF      FARG_Glcd_Box_y_upper_left+0
...
;deney14.c,118 :: 		someText = "8x7 Font";
	...
	MOVWF      _someText+0
;deney14.c,120 :: 		Glcd_Write_Text(someText, 5, 0, 2); // someText içeriði 5 e 0 a yazýlacak. ters renkte.
	MOVF       _someText+0, 0
	MOVWF      FARG_Glcd_Write_Text_text+0
	MOVLW      5
	MOVWF      FARG_Glcd_Write_Text_x_pos+0
...
;deney14.c,122 :: 		delay2S();
	CALL       _delay2S+0
;deney14.c,124 :: 		Glcd_Set_Font(System3x5, 3, 5, 32); //system3x5 yazý tipi. bunlar glcd kütüphanesinde belli.
	MOVLW      _System3x5+0
	MOVWF      FARG_Glcd_Set_Font_activeFont+0
